...
def build_object() -> tuple[bytes, bytes, int]:
    ...
    P_POS, P_NRM, P_UV, P_DL, P_PAL, P_MTX = 0x40, 0x60, 0x80, 0xA0, 0x100, 0x120
    ...
    data[P_PAL:P_PAL + 20] = struct.pack(">10H", *range(10))
    ...
    pairs = [(1, (0, 1)), (10, (0, 4)), (1, (P_MTX, 1)), (1, (0, 5)),
    ...
    for n in names:
        ...
        if n:
            strtab += n.encode() + b"\0"
    symtab = bytearray()
    values = [0, 0, 0, 0, 0, 0, 0, 0x300, 0x380]
    shndx = [0, 1, 0, 0, 0, 0, 0, 1, 1]
...
def test_parse_synthetic_object():
    ...
    assert obj.bones == ["Test__model1__.Hips"]
    ...
    assert pk.textures == ["Body"] and pk.palette == list(range(10))
    assert set(pk.mtx_slots.tolist()) == {3}
...
def test_plugin_detect_and_extract_without_textures():
    ...
    assert len(scenes) == 1
    s = scenes[0]
    assert len(s.primitives) == 1 and len(s.joints) == 1 and s.materials[0].texture is None
