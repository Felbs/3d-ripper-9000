...
| Konami TMNT 1-3: `AFS` archives (`TMNT.DAT`, named members), TMNT 2 / 3's `LPAC` packs of named RenderWare streams, little-endian RenderWare 3.x `.dff` clumps / worlds / `.anm`, and Konami texture packs (chunk 0x23: TMNT 1 name table + rwID_IMAGE, TMNT 2 rwID_IMAGE + rwID_TEXTURE) | Teenage Mutant Ninja Turtles 1, 2: Battle Nexus, 3: Mutant Nightmare (5 discs) | ripped through `plugins/renderware.py` + `plugins/afs.py` + `plugins/lpac.py` + `formats/konami_pac.py`; Mutant Melee open | [formats/konami-gamecube.md](formats/konami-gamecube.md) |
