...
| Avalanche DBL / DBU | Tak 1-3, Chicken Little, DBZ Sagas, Rugrats | sub-database container (`plugins/dbl.py`); mesh records = raw GX FIFO streams whose vertex-array formats are still unresolved | [formats/avalanche-dbl-gamecube.md](formats/avalanche-dbl-gamecube.md) |
...
| Konami TMNT 1-2: `AFS` archives (`TMNT.DAT`, named members), TMNT 2's `LPAC` packs of named RenderWare streams, little-endian RenderWare 3.x `.dff` clumps / worlds / `.anm`, and Konami texture packs (chunk 0x23: TMNT 1 name table + rwID_IMAGE, TMNT 2 rwID_IMAGE + rwID_TEXTURE) | Teenage Mutant Ninja Turtles 1, 2: Battle Nexus (3 discs) | ripped through `plugins/renderware.py` + `plugins/afs.py` + `plugins/lpac.py` + `formats/konami_pac.py`; TMNT 3 / Mutant Melee payloads open | [formats/konami-gamecube.md](formats/konami-gamecube.md) |
