...
| Blitz `.gcp` packs | Pac-Man World 3, Bratz, Bad Boys, Chicken Little, ... (9) | archive directory cracked: 337 named per-level members (0x800-sector offsets + byte sizes), members split into stamped packages; mesh arrays still open - the packs hold a type-tagged object stream, so floats are stored as `06` + 4 bytes and a contiguous-float scan is blind to them; the stampless `common_*` packs (1,121 on Bratz: Rock Angelz) are not split at all yet | `gcrip/plugins/blitz.py`, `gcrip/formats/blitz_gcp.py` | [formats/blitz-gcp-gamecube.md](formats/blitz-gcp-gamecube.md) |
