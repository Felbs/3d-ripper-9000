...
| `res` resource files (tagged sections, end-of-file directory) | Digimon Rumble Arena 2, Lemony Snicket, Samurai Jack: The Shadow of Aku (3,804 files) | split into labelled sections by `gcrip/plugins/res.py`; `surf` textures are CI8 with an `RGB565` palette at +32 and dimensions at +12, but the mip/level sizing is unsolved (44 of 345 sections fit), so nothing is decoded yet; `rdms` meshes still to decode | [formats/backlog-map.md](formats/backlog-map.md) |
