...
| Terminal Reality `_smf` static meshes | Blowout (version 7), BloodRayne (version 4) | models: GX display lists with INLINE vertices (quad opcode `0x84`); v7 is a 13-byte vertex (s16 pos * 2^-8, s8 normal, s16 uv), v4 a 16-byte one (s16 pos * 2^-15, Q1.14 s16 normal, u16 uv); degenerate triangles dropped and winding fixed from the stored normals | `gcrip/plugins/tr_smf.py`, `gcrip/formats/tr_smf.py` | [formats/terminal-reality-pod.md](formats/terminal-reality-pod.md) |  RoadKill's version 6 (~500 meshes) and Blowout's `_dfm` rigid-part characters are mapped but not decoded - both store f32 arrays instead of display lists.
