...
| `MDGC0200` blocks | Superman: Shadow of Apokolips (255 `.dgc`) | uncompressed; block type `0x1007` with a 64-byte header giving vertex and corner counts, then big-endian f32 positions, RGBA corner colours and s8 normals - 78 blocks / 9,328 vertices in 3 MB of one level.  The index list is not located yet, so nothing is extracted | | [formats/superman-mdgc.md](formats/superman-mdgc.md) |
