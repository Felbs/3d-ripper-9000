...
| Konami TMNT 1-3: `AFS` archives (`TMNT.DAT`, named members) of little-endian RenderWare 3.x `.dff` clumps / `.pac` worlds / `.anm` and Konami texture packs (`.txd` / `.pac` chunk 0x23 of rwID_IMAGE palettised images) | Teenage Mutant Ninja Turtles 1-3 (5 discs), TMNT: Mutant Melee | ripped through `plugins/renderware.py` + `plugins/afs.py` + `formats/konami_pac.py` | [formats/konami-gamecube.md](formats/konami-gamecube.md) |
