...
| Terminal Reality `_smf` static meshes | Blowout (version 7); BloodRayne's version 4 open | models: GX display lists with INLINE vertices (quad opcode `0x84`, 13-byte vertex = s16 position * 2^-8, s8 normal, s16 uv), degenerate triangles dropped and winding fixed from the stored normals | `gcrip/plugins/tr_smf.py`, `gcrip/formats/tr_smf.py` | [formats/terminal-reality-pod.md](formats/terminal-reality-pod.md) |
