...
| `.jam` archives - three formats: `FSTA` (High Voltage, members open `HVSI`), `JAM2`, `LJAM` | Billy & Mandy + Kids Next Door (`FSTA`, 155 files), Charlie and the Chocolate Factory, Hunter: The Reckoning | `FSTA` header, name and extension tables and 12-byte entries mapped; the per-extension group index is still open, so nothing is extracted yet - the extension list includes `TPL` | | [formats/jam-fsta-hvs.md](formats/jam-fsta-hvs.md) |
