...
| Nintendo U8 archives (`.arc`: `55 AA 38 2D`, 12-byte nodes + string table) | Harvest Moon: A Wonderful Life / Another Wonderful Life, F-Zero GX (`vehicle_parts/parts_all.arc`), Swingerz Golf, Ultimate Muscle, One Piece: Treasure Battle - 12 discs, 1,414 archives | expanded as a container, members keep their in-archive paths (`gcrip/plugins/u8.py`, `gcrip/formats/u8.py`) | - |
| Loose RenderWare textures (`.dds` / `.tga` / `.tgx` named after the material) | MLB SlugFest 2003 / 2004, Outlaw Golf, RedCard 2003 | bound by `gcrip/plugins/renderware.py` through the new `gcrip/formats/tga.py` (SlugFest 0 -> 100% textured, Outlaw Golf 0 -> 92%) | - |
