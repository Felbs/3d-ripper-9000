...
141 discs.  Used so far: Bratz (Blitz actors; `bUploadTexture` for the texture formats), Muppets Party Cruise (BOLT), The Sims 2 (Edge of
Reality), Mortal Kombat: Deception (SEC), NFS Underground (`Speed.elf`, DWARF 1 + symtab), Super Mario Strikers (`MarioSoccerR.elf` + its map - the DOL matches none of the maps), Medal of Honor: Frontline (`Moh2RelGC.elf` symtab), Rising Sun and GoldenEye: Rogue Agent (`MOH3RDVD.ELF`, `GE2RDVD.ELF` - `TLT_GetRelocationTable`, `RenderMoh3_*` vertex formats), Scooby-Doo! Mystery Mayhem (`engine_ret.elf` + `.MAP` - `EFRessourcesMgr::LoadLevel`, the `RegisterDynamicClass` ids), Scooby-Doo! Unmasked and Scaler (`engine_ret.elf` symtab - the `DTBinaryPersistStream` loaders transcribed), Smashing Drive (`smash.elf` DWARF 1 - `s_model` and its mesh / vertex / command / texture structs are the file), BloodRayne and Blowout (`bloodrayne.elf`, `Blowout.elf` symtabs - `CDeformableModel::loadStreamBinary`, `APIDLLpolyListGCBoneVertex` paired-single loads settled the `_dfm` vertex record), 4x4 Evo 2 (`4x4.elf` symtab - `C3DModel::loadBinary`, `setVertexFormat` naming the `SGCPacketHeader` fields), RoadKill (`Hunter.elf` symtab - `CModel::loadHeader` for the version-6 `.smf` / `.smb`), Casper (`casperGCN.elf` symtab - RenderWare's own names showed the `.hff` to be RW streams), Jimmy Neutron: Jet Fusion (`Jimmy.elf` symtab - `Model::ExploreBuildVertex` named the GC01 vertex), Hot Wheels World Race (`HotwheelsFCDntsc.map` over `main.dol` - `cLZSS::Decompress` settled the `.bad` framing, `cFile::LoadArchiveDir` the `.bah`, `cBogFile` / `cModelFile::Init` the textures and models), Ultimate Spider-Man (Treyarch NGL - its `symbolgc-final.map`
is a binary `SYM1` table: 16-byte records of address, name start, name end, size, then the
names), 007: Agent Under Fire (`Bond.elf` symtab - `RSRC_Load` / `BOA_ExpandInplace` / `R_InitSurfaces` / `R_DrawSurf` / `SHDR_FixupShaderTable` gave the map chunks, codec, world vertex layout and shader-to-texture path), TimeSplitters 2 (`main.dol`, no symbols - the draw routine that calls `GXSetArray` nine times, found by that count, gave the `gcr` node record), TimeSplitters: Future Perfect (`main.dol`, no symbols - the `GXSetVtxAttrFmt` table settled the s16 fractions, the draw routine the palette normals and the 0xc0 record; Second Sight's DOL carries the same normal palette), WWE Day of Reckoning (`main.dol`, no symbols - the strip loop writing to the write-gather pipe at `0x800bdb2c` settled the version-4 YOBJ corner), and the DOLs of Madden 06, Tiger Woods 2005, Frogger and Billy & Mandy (no symbols -
