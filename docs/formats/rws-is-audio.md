...
## Piglet's `PIGGCN.pkd` - container solved, payload sizing not (2026-09-01)

232 MB, entropy 7.99, and it opens `78 da` - **a chain of independent zlib streams**.  Each one
terminates cleanly (`eof`), and the next begins where `unused_data` starts; hundreds walk in a
row.  Inflating the first 400 gives a clear census of what the disc is made of:

| member magic | count | what it is |
|---|---|---|
| `1b 00 00 00` | 163 | RenderWare `0x1B`, animation |
| `XMD\0` | 131 | property data - `AttachedSND`, `BoneLink`, `Rigid`, `Sphere`, `PitchVariation` |
| `1e 00 00 00` | 40 | RenderWare `0x1E` |
| `10 00 00 00` | 23 | RenderWare **CLUMP** |
| `16 00 00 00` | 20 | RenderWare **TEXDICTIONARY** |
| `DSBH` | 5 | unidentified |

plus `BM` bitmaps and `METR` further in.  So the geometry is ordinary RenderWare, as on
Madagascar, and `plugins/renderware.py` already reads three of the CLUMPs for **670 triangles**
without any new code.

**`XMD\0` is not `XMDL`.**  `gcrip/formats/xmdl.py` wants the four bytes `XMDL`; this is three
letters and a NUL, a different format, and `is_xmdl` says so.

### The open problem: most RenderWare members are a few bytes short

Of the 23 CLUMPs, only 2 satisfy `12 + size == len(member)`; the rest declare a chunk slightly
larger than the member holds - 704 bytes holding a chunk claiming 720, 13,728 claiming 13,772,
610,528 claiming 610,943.  The shortfall is not constant and not a fixed fraction.

Two explanations were tested and **both fail**:

* *the blocks are one logical stream* - concatenating 600 inflated blocks gives 22.5 MB that
  does **not** walk as a chunk chain: the first word reads as type `0x444d58`, ie the `XMD` text,
  so the members are individually meaningful and not slices of one file;
* *a chunk continues into the next member* - joining the two does make the size fit, but the
  next member is a different asset, so this is a coincidence of size rather than evidence.

### CRACKED: the assets span blocks

`gcrip/formats/piglet_pkd.py` + `gcrip/plugins/piglet_pkd.py`.

The give-away was in the numbers already collected: **every short block's length is a multiple
of 16, and the two that fitted exactly are not.**  Blocks are padded to 16 except an asset's
last one, so an asset simply continues into the following block.  Concatenate the blocks into
one image, read each asset at its block start, and **13 of 13 clumps parse** in the first 300
blocks where 2 of 23 did before.

The chain covers the archive **exactly** - 232,370,273 of 232,370,273 bytes in 10,328 streams,
425 MB inflated - which is what says the walk is right.  Counted over all of them::

    4,891  0x1B  animation        936  0x10  CLUMP
    3,089  XMD   property data    404  0x16  TEXDICTIONARY
      533  0x1E                    66  0x0B  WORLD
      162  DSBH                   140  0x0C

**A performance trap worth keeping**: slicing `bytes` to advance through the chain copies the
remaining 232 MB on every one of the ten thousand blocks.  The first walk never finished and had
to be killed; with a `memoryview` it takes seconds.

### What it actually yields, measured over the whole archive

**6,969 members**: 4,891 `anm`, 935 `dff`, 533 `0x1E`, 404 `txd`, 140 `0x0C`, 66 `bsp`.  Of the
1,001 clumps and worlds, `plugins/renderware.py` claims all 1,001 but returns a scene for only
**68 of them - 27,478 triangles**.

That is a correction to what an earlier sample suggested.  Thirteen of thirteen clumps *parsed*
in the first 300 blocks, which is what made "the payloads need no new reader" look true; over
