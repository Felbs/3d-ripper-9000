...
`XMDL` - 69 members of 4.4 MB - is the model side, and it is chunked and readable-looking:
`XMDL` `NTGC` then `MDEL`, `MATR`, `TXNM`, `MGRP` sections, 43 models in the first 2 MB alone.
That is the obvious next target in this cluster.
