...
| Neko NGDK `.GCN` textures and `DYNO` props (Cocoto x3, Charlie's Angels) | 4 | **LZ and the `MWLD` static world read (2026-09-04)** - Okumura LZSS container, 20-byte faces with object-relative indices, 16-byte s16 vertices with uv and colour (`gcrip/formats/neko_lz.py`, `neko_mwld.py`; wave 61).  Open: the texture directory for the headerless `GFX.PC`, and `DYNO` (a serialised object tree with `0xcccccccc` fill) where the props live | [formats/neko_lz.md](formats/neko_lz.md) |
