...
|---|---|---|---|
| EA content on Tiger Woods 2003 / 2004 / 2005 | 3 | **`OBG` terrain and `TXG` textures are both done on Tiger Woods 06** - 4,814,090 triangles and 2,073 textures (see [formats/ea-obg.md](formats/ea-obg.md), [formats/ea-txg.md](formats/ea-txg.md)).  The other three discs carry **neither** in their first twelve archives - their kinds are `Cact`, `RPNS`, `RLst`, `CAMC`, `tLOD` - so their content is laid out differently and finding it is the open question.  Also open: 61 of 89 `OBG` members produce no scene, having no finite position array or too few triangles | |
