...
triangles, textured by gct id.  See [formats/free-radical-pck.md](formats/free-radical-pck.md).
Wave 55 re-rips TimeSplitters 2.
...
| Free Radical `gcr` **levels** (TS2 `bg/level*.gcr`) and the **Future Perfect / Second Sight** layout | 3 | **TS2 characters, props and guns closed 2026-09-03** (below): in a story pak 71 of 72 `.gcr` are that flavour, the 72nd the level.  `level11.gcr` (852 KB): `+0` -> 62 texture slots at +0x20 (`u32 gct id, u32 flags, 0, u32`, ended by 0xFFFFFFFF), `+8` -> a list of 98 room records (0x48 bytes: `u32 2, u32 1, f32 1.0, ..., f32 bbox`), `+0xc` -> a table of small records with radians; the geometry after +0x410 is **u16 / s16 streams**, not the f32 arrays of the characters, so it is quantised and has its own draw routine (the `GXSetArray` caller with a 12-byte position stride at `0x801b9f54` is the candidate).  Future Perfect's props (`trolla_chair_prop.gcr`: `12, ptr header, ptr, ptr`) embed their textures behind the slot table (`ptr, u32, 0, 0x10000000`) and end in s16 data, its characters (`trolla_jp_chr.gcr`) open with three pointers into a 52-byte block; both need the FP DOL | TS2 levels: read `0x801b9f54` in the TS2 DOL (scratchpad `ts2/sys__main.dol`); FP / SS: pull their DOLs and find the `GXSetArray` caller that sets an s16 position format |
