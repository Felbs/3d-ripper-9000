...
| format | discs | state |
|---|---|---|
| Visual Concepts `DAT` | **5** (NBA 2K2, NBA 2K3, NFL 2K3, NCAA Basketball 2K3, NCAA Football 2K3) | **the largest cluster left**: each disc is nine files, one of which is a 0.8-1.3 GB `game.dat` holding the whole game.  **Container fully mapped** - entry count at +32, 24-byte entries from +36 (`counter | name hash | kind | 0 | offset | size`), then a NUL-terminated **name list** of exactly `count` names, then the members from the next 32-byte boundary, each opening with the same twelve bytes.  Members are named, and the 1,916 entries marked `0x01000000` are exactly the 1,916 `.IFF`; the other 52 (`.DAT`, `.BIN`, `.CDF`) **need no codec and could be extracted today**.  Blocked on the `.IFF` payload: bit-packed, and a sweep of plain LZSS (both bit polarities, both bit orders, both endiannesses, 11/12/13 offset bits, length +2/+3, header skip 0-19) hits the exact output length on none of 37 members, as do LZ4 and every decoder gcrip has | see [formats/visual-concepts-dat.md](formats/visual-concepts-dat.md) |
