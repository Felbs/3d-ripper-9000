...
| format | discs | state |
|---|---|---|
| Pokemon `FSYS` `LZSS` codec | 2 (Colosseum, XD) | the container ships (`plugins/fsys.py`) and members come out named, but **2,257 of 2,414 sampled members are `LZSS` and all of XD's are**.  The header gives an exact unpacked-size oracle and 48 ring-buffer LZSS parameter sets already reach it, collapsing to four outputs that all begin with ring fill - so length alone does not discriminate | start from the first operations of a stream, not from a length sweep.  See [formats/fsys.md](formats/fsys.md) |
