...
| Darkened Skye `.skg` skeletons | 1 | `SKX` models ship (255 of 255, 135,749 triangles) and their skinning records give a joint index and weight per influence, but the positions are in **joint space** and there is no skeleton in the `SKX`.  The 17 `.skg` open `\0GKS` and are **now partly read (2026-09-02)**: +8 is the file own length on 17 of 17, the model name follows at +12, and there are **788 `SKT` records at an 84-byte stride** - 140 of them in `BoneSkye.skg`, none at all in five of the files.  One section is **20-byte animation keys, a frame number and a unit quaternion**: over 400 consecutive records the frames run 0-90 and all 400 quaternions are unit length (358 distinct, sd 0.0000).  That is one section layout and not the file - read whole that way only 3.0%% are unit length - so the block bounds are still unread, and the bind pose that would fix the joint-space positions is most likely in `BoneSkye.skg` `SKT` records.  Until they are bound the exporter takes the first influence, which is coherent but not exact on multi-joint models  **2026-09-04 evening, from the cached .skg set (zero disc reads): the file is fully mapped.**  Every `.skg` is an animation GROUP: header section table at +0x2c; a clip directory (name + `SKT` records: fps 30, duration, frame count, bounding sphere); **the skeleton** - 64-byte records at stride 64 found by their constant ±5 bbox signature, 75 of them in BoneSkye = `{i32 parent (-1 root), u32 index, i32 mirror, f32 length, f32[3], f32[3] quat-xyz (w derived), f32[6] bbox}` - a real hierarchy (spine chain 0-6, arm branches); **the key streams** - kind-2 `SKT` descriptors give 3 channel groups of `{u32, u32 nkeys, u32 joint, u32 offset}` track headers, translation keys `{f32 frame, x, y, z}` (16 B), rotation keys `{f32 frame, quat xyzw}` (20 B, 100% unit), tiling exactly; and an **embedded skinned mesh** per group (the SKX vertex record format: verts/remap/normals/uvs).  **What still fails: the transform composition.**  A 32-candidate sweep (T*R vs R*T x field roles x w sign x joint mapping x length-along-x) scored by mesh edge length leaves the best at median edge 16%% of the model extent - spaghetti.  The joint-space -> model-space convention needs the game's own composition code; the disc's DOL is the next read | see [formats/darkened-skye.md](formats/darkened-skye.md) |
