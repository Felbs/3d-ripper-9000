...
| Free Radical `gcr` meshes, and the `gct` palettes | 3 (TimeSplitters 2, Future Perfect, Second Sight) | the container ships and `gct` textures decode for the `CMPR` and `RGB5A3` codes (3,598 of 6,465 sampled).  **The palette-indexed codes (4, 5, 7, 10, 11, 12) have no palette in the file** - at 8 bits a pixel `I8` and `IA4` both draw a sniper scope as vertical banding, so the palettes live elsewhere in the archive; finding them is worth about 2,900 more textures.  `gcr` is the geometry and is untouched | see [formats/free-radical-pck.md](formats/free-radical-pck.md) |
