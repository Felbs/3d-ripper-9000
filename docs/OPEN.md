...
|---|---|---|---|
| A2M `.gc` textures | 5 (Teen Titans, Monster House, Ed Edd n Eddy, The Ant Bully, Happy Feet) | **the meshes ship** (`plugins/a2m_gc.py`) - see [formats/a2m-gc.md](formats/a2m-gc.md).  What is left is the textures: no resource type has been identified as image data, so meshes export with vertex colours and no maps | types 24 (the 1.29 MB environment), 34 (185 props) and 79 (163) are where to look |
