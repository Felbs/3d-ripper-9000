...
| format | discs | state |
|---|---|---|
| Pokemon `FSYS` member formats | 2 (Colosseum, XD) | **the container and the `LZSS` codec both ship** - 800 members decode to their declared size with 0 failures, 1,936 out of the 16 largest archives.  What is left is the member formats: no plugin claims one yet | the u16 pairs at the head of many members (42x84, 200x150, 84x84) read as dimensions, so textures first.  See [formats/fsys.md](formats/fsys.md) |
