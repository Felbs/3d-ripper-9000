...
| Warthog skinned character meshes | 3 (Animaniacs, Looney Tunes, Harry Potter and the Sorcerer's Stone) | **the archive and its codec are solved** (`plugins/wart_hog.py`, 98-99% of members on all three discs) and the members are named and decompressed.  They share an 8-byte header - `u32 id`, `u32 kind` - with kind 3 = `.btga`, 10 = `.bmsh`, 1 = `.bskl`, 2 = `.banr`.  **`.btga` now ships** (`plugins/wart_btga.py`, 26 of 26 on `frontend.hog`).  **`.bmsh` ships too** (`plugins/wart_bmsh.py`, 13 of 13 tabled members, each confirmed against the header's bounding volume).  Left: the **skinned character meshes**, which carry no section table at all - `brainbodyshape`, `pinkybodyshape` and `brainheadshape` in the sample - so the chain identity that locates every other mesh finds nothing in them | see [formats/warthog-hog.md](formats/warthog-hog.md) |
