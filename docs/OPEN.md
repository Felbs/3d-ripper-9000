...
| Hunter `AGG` meshes | 1 | the `LJAM` container ships (22,662 members, 304 of 304 MB), and the geometry inside is **plain ASCII** - 2,114 `AGG` files opening `Mesh[1]` in a brace-block syntax shared by the shaders, materials, lights and animation beside them.  No codec, just a grammar | see [formats/high-voltage-ljam.md](formats/high-voltage-ljam.md) |
