...
| High Voltage `GMS` | 3 (Billy & Mandy, Kids Next Door, Charlie and the Chocolate Factory) | header readable, size field exact; **payload entropy 7.73 behind a 2.39 header** - compressed, and not zlib / Yaz0 / Yay0.  Charlie's `JAM2` archives add **1,097 more `GMS`, 1,115 `GKA` and 1,204 `GGG`**, a third independent corpus to work the codec against  **2026-09-02, corrected**: the entropy argument does not hold.  The payload's byte histogram scores chi-square **5,282** against **310** for a zlib control of the same bytes, so a strong general-purpose compressor is ruled out; and at stride 8 one column in eight holds **79 distinct values** against 238-248 for the others (248 in a shuffled control), with its **top bit clear on 99.8%** of records.  A constrained byte every eighth is a flag byte or a record tag - start from the group of eight, not the bit level |
