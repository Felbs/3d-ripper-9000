...
| `res` characters (`bmsh`) and the `node` placement | 3 (Samurai Jack, Lemony Snicket, Digimon Rumble Arena 2) | **textures bound 2026-09-04** (rdms +4 -> gshd, gshd +0x5c -> surf, self-relative; wave 58).  Still open: the **characters** - `characters/*.res` hold `bmsh` skinned meshes (0x28-byte batch records linking their gshd, a bone-index list, s16 positions, no GX display lists - a CPU-skinned index list) with `body` bind blocks and `banm` animations, unread on all three discs; and the `node` transform for level assembly (the 52-byte link record's six floats are not the mesh bounds) | see [formats/res-rdms-meshes.md](formats/res-rdms-meshes.md) |
