...
| Acclaim `.GDF` / `.SKN` meshes | 3 (All-Star Baseball 2002, 2003, 2004) | the textures ship (42,403 of them) and the geometry is beside them: `.GDF` (194 files, 111 MB on 2003) and `.SKN` (50) both open with a 20-byte NUL-padded name and a big-endian count, then records carrying 16-byte material and texture names (`two_tone_bat`, `bat`, `bat_shadow`) that match the texture names the reader now produces | see [formats/acclaim-asb-textures.md](formats/acclaim-asb-textures.md) |
