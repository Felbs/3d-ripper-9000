...
| Cocoto `.GCN` / `.pc` (Neko Entertainment) | 3 (Kart Racer, Funfair, Platform Jumper) | **the LZ is cracked (2026-09-04)** - `u32 packed, u32 unpacked` then Okumura LZSS (flags LSB-first, 1 = literal, refs = absolute 12-bit ring index + 4-bit length, ring written from 4,078; Baten Kaitos's text files use the same routine bare) (`gcrip/formats/neko_lz.py`, container plugin `neko_lz`); every level file on the three discs unpacks to its declared size.  The unpacked `.GCN` is one `MWLD` chunk (`3, 4, bytes, "MWLD"`, then `00 01 dd 0c, 00 00 09 cf, 00 09 cf 94 ...` and a stream of `20 09 cf 94 01 xx` records) - **no GX display lists, no float runs**: a quantised serialised world that needs its own reader; `GFX.PC` is raw CMPR-looking texture data with no header, `.cp2` (uncompressed, `00 00 00 64`) names the placements (`Node_Start_Battle`, `MAP_CORNER`, `TORCHE`).  The DOL is stripped; the `%sL%d\L%d.GCN` loader is the place to start | [formats/neko_lz.md](formats/neko_lz.md) |
