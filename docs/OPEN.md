...
| Free Radical `gcr` **level / tile** flavour and the **Future Perfect / Second Sight** layout | 3 | **TS2 characters closed 2026-09-03** (below).  `bg/level*.gcr` and `ob/tile*.gcr` open with two pointers and a float grid (`0x4ff04, 0x4fedc, f32 ...` on `ts2_model.gcr`: the first a 16-byte texture-slot table, the second a `u32 8, ptr x4, counts` block); Future Perfect's `trolla_jp_chr.gcr` opens with three pointers into a 52-byte header (`12, 0x5dbc, 0x8214, 7, ...`) and starts with **s16** data, so its vertices are quantised and its textures embedded | the same draw routine in the TS2 DOL takes the runtime node record, so the level flavour is "find what builds the 0xa0 records"; for FP / SS pull their DOLs and look for the `GXSetArray` caller with the s16 position format |
