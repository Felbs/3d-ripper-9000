...
| Warthog `.btga` / `.bmsh` resources | 3 (Animaniacs, Looney Tunes, Harry Potter and the Sorcerer's Stone) | **the archive and its codec are solved** (`plugins/wart_hog.py`, 98-99% of members on all three discs) and the members are named and decompressed.  They share an 8-byte header - `u32 id`, `u32 kind` - with kind 3 = `.btga`, 10 = `.bmsh`, 1 = `.bskl`, 2 = `.banr`.  **29,047 textures and 19,156 meshes** behind ordinary format work, no codec left | see [formats/warthog-hog.md](formats/warthog-hog.md) |
