...
| Taz: Wanted Blitz actors | 1 | the `.gcp` packs expand to 697 members (267 `.tba`, 430 `.tbt`) but every actor reads 0 nodes / 0 meshes: vertex_type 0 (not in `blitz_actor.LAYOUTS`) and the node-tree root pointer at +160 does not resolve for this variant (the working readers use SOFT_AT 32 / ROOT_AT 160; on Taz those fields hold floats and mismatched pointers).  The display lists are found by scanning - 521 `0x98` strips, stride 8 = `LAYOUTS[8]` (pos/nrm/clr/tex u16) - so the geometry is standard; only the actor's node / mesh header offsets differ.  A bare `0x98` scan is **not** a shortcut: the fields at SOFT_AT 32 (`pos_at 0x11700`, `npos 3232`) do not point at real s16 positions (the array reads as full-range noise) and unanchored `0x98` scanning matches garbage (corner indices to 65534).  The whole header must be re-mapped from `taz.elf`/`main.dol` first.  Other Blitz `.gcp` discs (Bratz, SpongeBob CFTKK) read, so it is a Taz-specific actor layout |
