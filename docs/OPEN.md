...
| Spawn / Scorpion King `PHM` / `PAT` / `PHA` members | 2 | both discs' `.wad` now open and all 12,018 `TIM` textures decode, but 8,000-odd members are `PHM`, `PAT`, `PHA` and `SOB` and nothing reads them - that is where any geometry would be | see [formats/spawn-toc-wad.md](formats/spawn-toc-wad.md) |
