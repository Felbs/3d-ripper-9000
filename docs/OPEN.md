...
| Avatar / Jimmy Neutron `pack` members | 2 | with the shadowing fix `thq_pack` now opens Avatar's nine archives (28 members in one 81 MB pak), but the members are `.rad` objects (`rad0` + a section table) that nothing reads yet.  Jimmy Neutron's 23 `pack` archives are a **different variant** - `u32 0` at +4 where Avatar has `u32 1` - so `thq_pack` does not claim them at all | |
