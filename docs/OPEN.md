...
| Warthog `.hog` codec - the high form's literal count | 3 (Animaniacs, Looney Tunes, Harry Potter and the Sorcerer's Stone) | `0xE0|n` = literal run of `(n+1)*4`; `b < 0x80` = match with `lit = b & 3` emitted first, `len = (b>>2)+3`, `off = next+1`; **`b >= 0x80` = two operands with `len = (a>>4)*2+3` and `off = b+1`**, under which `frontend_cog1.lvl` comes out with its first 130 characters exactly right - the `s`/`t`/`b`/`o` count series in order, no repeated fragment.  **All that is wrong is the literal count**: `(t>>2)&3` is right for `0x87` (needs 1) and wrong for every token above it - `0x89`, `0x8b`, `0x9c` and `0xaf` all need 0 and it claims 2, 2, 3, 3, so they swallow `0xe0`/`0xe1` literal-run tokens as if they were text.  Either the count comes from elsewhere or `0x87` is not a high token.  Behind it: 29,021 `.bmsh` meshes and 36,156 `.btga` textures | see [formats/warthog-hog.md](formats/warthog-hog.md) |
