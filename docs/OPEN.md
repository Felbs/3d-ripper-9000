...
| Climax `.bad` archives | 3 (ATV: Quad Power Racing 2, Hot Wheels World Race, The Italian Job) | each disc is **one `.bad` plus HVQM video and DSP audio**, so the archive is the whole game - 94, 130 and 76 MB.  Header: `u32 1`, `u32` (15,696 / 32,936), then `0xff` and an 11-character build tag (`CUBAN 1._02`, `BOG 1.01`); Hot Wheels differs, with `u32 0`, `u32 728`, then `HSC`.  **No directory found** at the head, the tail, or at the field, `size - field` and `field * 4` offsets.  The body is high entropy, but ATV's **last ~15,696 bytes are plain game text** ("You failed to perform a bicycle"), which is what the second field appears to measure on that disc though not on The Italian Job.  Treat as probably packed, and note three codecs already resisted on 2026-08-30 | |
...
| Spawn `PHM` / `PAT` / `PHA` members | 1 | the `TOC` container ships and all 5,919 `TIM` textures decode, but 5,704 members are `PHM` (2,071), `PAT` (2,031) and `PHA` (1,602) and nothing reads them - that is where any geometry would be | see [formats/spawn-toc-wad.md](formats/spawn-toc-wad.md) |
| The Scorpion King `.wad` | 1 | 200 archives, 312 MB, a **different layout** from Spawn's - no `TOC`, only the first 32 bytes parse as an entry.  144 are named sound banks (`<name>SFX`); the other 56 include `bonus*.wad` whose first record names a `TIM` | see [formats/spawn-toc-wad.md](formats/spawn-toc-wad.md) |
