...
| format | discs | state |
|---|---|---|
| Pokemon XD `FSYS` member wrapper | 1 (XD) | **Colosseum is done** - 1,332 textures (`plugins/fsys_tex.py`).  XD's members open with their own length as a big-endian u32 followed by two more sizes and a count, so its images sit one layer further in and none of Colosseum's pattern matches | see [formats/fsys.md](formats/fsys.md) |
