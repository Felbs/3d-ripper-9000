...
(10k-26k triangles each) and dm1's 124 `restable` models (25,827 triangles).  Patches, lightmaps and placements are still open - see the row below and
...
node records before a trailer the file's `+4` points at), TS2 levels (the same records
without the trailer, found by their +0x9c back-pointer), Future Perfect and Second Sight
(0xc0 records, `s16 / 1024` positions, normals from the 4096-vector palette both DOLs
carry - shipped as `gcrip/data/frd_normals.npz` - textures embedded or named by hash),
and the array-block characters (TS2 `chrinc`, FP / SS characters).  Every sampled file on
all three discs reads.  Waves 55 (TS2) and 56 (FP, Second Sight).  See
[formats/free-radical-pck.md](formats/free-radical-pck.md).

## Closed 2026-09-03: TimeSplitters 2 characters (Free Radical `gcr`)

The "where does the position array start" question had the file upside down: a character
`gcr` is the runtime image and its header is the **last 52 bytes**, which `+4` of the file
points at.  The 0xa0-byte node records precede it, each naming its position / uv / normal /
colour arrays and, per batch, a GX display list of 8- or 9-byte index vertices (the ninth
byte a matrix index on the skinned nodes).  Read from the TS2 `main.dol` draw routine (the
`GXSetArray` caller at `0x8024a454`).  All 314 characters in `chr.pak` read, 343,841
triangles, textured by gct id.  The levels are the same records without the trailer
(`level_nodes` finds them by the back-pointer at +0x9c): level 11 is 51 sectors and 10,461
triangles.  See [formats/free-radical-pck.md](formats/free-radical-pck.md).  Wave 55 re-rips
TimeSplitters 2.
...
symbols, and the `.hff` is a run of RW streams with nothing but the PNGs between them.  The
container now walks chunk headers and the existing RenderWare plugin reads the worlds and
clumps.  See [formats/hff-carving.md](formats/hff-carving.md).
...
Wheels' `HotwheelsFCDntsc.map` names `cLZSS::Decompress`, which inflates **one block** -
`u32 kind, u32 count` - to exactly `count` bytes with a fresh ring, and the archive is a block
a member.  The decoder that ran on from block one was inflating block two's header as data.
...
| 007: Agent Under Fire - what `maps/*.ngc` still holds | 1 (+ Everything or Nothing / From Russia with Love if the studio kept the shape) | **the world and the models ship 2026-09-03/04** (`plugins/auf_ngc.py`: BOA chunks, `ngcsurfs` strips through the world matrix, shaders -> `restxtrs` textures, the `restable`'s `.gcm` `NGCObject3D` images).  Left: bicubic patches (`R_DrawPatch`), the `ligtmaps` images, `bmodels` + entity placements, skinning | [formats/auf-ngc.md](formats/auf-ngc.md) |
...
| format | discs | state |
|---|---|---|
| Asobo `.dgc` (Ratatouille) | 1 | **Asobo Studio "Internal Cross Technology"**.  Container mapped: 24-byte big-endian directory at 0x120 (`type | uncompressed | stored | block size | hash`), payload back to back, and **raw when uncompressed == stored and block size is 0** - 16 of 55 records, 29% of the archive, needs no codec.  Uniform 150/160 KB chunks mean it is a **paged virtual file system**, so cracking the codec yields an address space, not files - the name-to-page directory is a second problem.  See [formats/asobo-ict-dgc.md](formats/asobo-ict-dgc.md).  **2026-09-03**: `ratsgc_m.elf` has symbols - the resource LZ (`UnPack_Z::DecodeRS`, 30-flag control words) is transcribed in the note, but the page records use a different framing and the class serialisation above them is the real cost |
| Darkened Skye `.skg` skeletons | 1 | **CRACKED 2026-09-04 sprint** - see [formats/darkened-skye.md](formats/darkened-skye.md): SKX carries its own joint table (+0xa4, 128B rows), parents from the group .skg, vertices are s16 /1024 fixed-point in the joint frame (the old radius scaling was the bug - every skinned SKX in the library was garbage until tonight), G=parent@(R|t), render-verified body. Wave 67 re-rips the disc | see [formats/darkened-skye.md](formats/darkened-skye.md) |
| Visual Concepts 2K3 residuals (Sega Sports engine) | 5 (NBA 2K2/2K3, NFL 2K3, NCAA x2) | **container, codec, textures and now the SCNE MODELS all ship** (2026-09-04 night 2, `vc_scene`: ball, players, referees, backboards, net, coach parts, crowd - see [formats/visual-concepts-dat.md](formats/visual-concepts-dat.md)).  Open: (1) skinned colored-layout characters (the coach class) - INDEX16 body draws span several CP array-base repointings inside one index window, so parts place right but body seams spaghetti; needs the node graph's real array pointers; (2) mesh->texture binding + skeletons (same unmapped node graph, scenes ship with textures unbound); (3) the `HTXT`/`NAME` jersey-lettering layout (each team member's `unif` texture already extracts); (4) `PLAY` playbooks, `AANM`, `CSS`, `ROST`, `HiSn/HiFt/HiAu` families - decoded, unread; (5) `vc_pack.is_packed` misses tags with non-alpha chars (`" SSC"`) and streams whose first flags byte is not 0 (6 of 370 members) |
| Acclaim `.SKN` skinned meshes | 3 (All-Star Baseball 2002, 2003, 2004) | **CRACKED 2026-09-04** (`gcrip/formats/acclaim_skn.py` + plugin) - the radius sweep found nothing because `.SKN` has no f32 anywhere: s16 fixed-point, and every vertex is already **model space** (runtime matrices are `boneWorld x inverseBind`).  Header tiles the file to the byte on 17 of 17 samples; each geom is a rigid GX copy (one bone per vertex via `0x20` loads) plus a blended baked-bind-pose copy (`uv == pos + verts`, rows sorted by weight count, `w1+w2+w3 == rows`); players, hands and mascots render T-posed with named meshes and materials.  Still open here: the per-bone blend tables (they live with the animation data, not in `.SKN`), bone rest transforms / `.ANIM`, and binding the `asb_tex` textures by material name | see [formats/acclaim-gdf.md](formats/acclaim-gdf.md) |
...
| Gun `.ngc` | 1 | **The `.mpk` map packs are CRACKED (2026-09-04)** - `gcrip/formats/gun_mpk.py` + `gcrip/plugins/gun_mpk.py`.  The 315 content packs carry the level geometry: a scene header (44 bytes before the last `aa ff ee ff` marker: u32 position count, u32 normal count, u16 colour count, u16 uv count), four global arrays found from the normal array (the longest run of unit s16 triples, /16384; uvs s16 /1024 end 2 bytes before it, RGBA8 colours before those, f32 positions before the colours, base settled by a bounding-sphere vote), and meshes scanned by the `00 15 00 04` signature with the display-list size repeated at -36 and +12 - each a VAT-7 GX display list of absolute `(pos, nrm, col0, tex0)` index16 corners (the per-attribute index maxima equal the array counts minus one).  **Proof: every kept mesh's vertices sit 100% inside the bounding sphere its own header stores** (worst dmax/r = 1.017 over 692 meshes), and the renders are the levels - z_fort's walled compound with watchtowers, z_steamboat2's river valley.  28,085 / 31,878 / 27,592 triangles from the three verified members.  The earlier "index pairs" reading was these corners seen without their opcodes; the "stride-24 floats" were the pos array seen at the wrong phase.  **Still open**: (a) prop objects at the front of each pack - same mesh records over per-object INLINE arrays (rejected by the sphere test, 10-103 per pack); (b) binding the embedded CMPR textures (material checksum -> texture checksum lives in a 32-byte material table; the `04 20` image chain carries the pictures); (c) the 32-byte descriptor/group table right before the pos array (717 records whose u32 counts sum to more corners than the meshes draw - LODs?); (d) `.img.ngc` format 6, still unreconciled; (e) the 1,233 `.apk` asset packs | see [formats/gun-ngc.md](formats/gun-ngc.md) |
...
| `res` `node` placement | 3 (Samurai Jack, Lemony Snicket, Digimon Rumble Arena 2) | **textures bound 2026-09-04** (rdms -> gshd -> surf, self-relative; wave 58) and **characters read 2026-09-04** (`bmsh`: bind-pose buffer, one `0x99` strip, blended slots from their runs - `gcrip/formats/res_bmsh.py`).  Still open: the `node` transform for level assembly (the 52-byte link record's six floats are not the mesh bounds), and the `body` / `banm` skeleton and clips (bone records read, not exported) | see [formats/res-rdms-meshes.md](formats/res-rdms-meshes.md) |
| Neko NGDK `DYNO` props (Cocoto x3, Charlie's Angels) | 4 | **LZ, the `MWLD` static world and now the textures read (2026-09-04)** - `TIN.PC` is the slot / material / texture index the DOL loader reads, `GFX.PC` the pictures (CMPR, RGBA8, and C8-with-palette on Charlie's); wave 61.  Open: `DYNO` (a serialised object tree with `0xcccccccc` fill) where the props live | [formats/neko_lz.md](formats/neko_lz.md) |
| Fang `.gmo` model headers (Freaky Flyers) | 2 | **the `.mst` container and `.gtx` textures ship (2026-09-04)**; the models and worlds reach the rip through `gxscan` (display lists in the clear).  Open: the `.gmo` pointer image (absolute `0x0b1e....` pointers, base unknown) that would name meshes and bind textures | [formats/fang-mst.md](formats/fang-mst.md) |
| Acclaim `supertree0.tre` models (Vexx, Turok: Evolution) | 2 | **container and textures ship (2026-09-04)**: a hash-sorted table tiles the whole file; textures decode.  the tree nests (105 sub-tables, walked now) and the models are the 74 `AAAp` members (116 MB) - packed data behind 0x50-byte segment records, unread | [formats/acclaim-supertree.md](formats/acclaim-supertree.md) |
| Ubisoft Montreal 2000-02 24-byte geometry (Batman: Vengeance `.flt`, Donald Duck: Goin' Quackers and Tarzan Untamed `geoobj.bin`) | 4 (+ Rise of Sin Tzu placements) | **the `.fat`/`.000` archive reads on both discs and Sin Tzu's `geoobj.bin` models + `.tsd` pictures ship (2026-09-04; wave 63)**.  Open: Vengeance's `.flt` (24-byte vertex records, `^VisualMaterial:` names), Sin Tzu's `superobjects/` placements and the material chain for pictures not named after their material | [formats/ubi-fat-sintzu.md](formats/ubi-fat-sintzu.md) |  **Probed 2026-09-04**: a geometry record is `ff ff ff ff` + header, then N x 24-byte vertices (`f32 pos[3]` bbox -7.2..4.9 is character-sized, then `f32[3]` that is **not** a unit normal, 0.57-1.87 - a second position or tangent), then index groups (`u16 count, count x u16`, indices under the vertex count).  The positions are solid, but no empirical index reading works: the four groups of record 0 (7,6,4,9) triangulated as strips give edges ~30%% of the mesh extent (incoherent), so they are not strips into these 16 vertices - there is likely a vertex remap (a u32 list 1,2,3,4,5,6,... follows the u16 groups). and the second float triple is unidentified - the `GeoObj` reader is behind a C++ factory (loader 0x8014d7d8 -> vtable lookup 0x8014cccc, called via function pointer with no direct callers), so **static tracing cannot reach the deserializer** - it needs dynamic analysis (an emulator breakpoint on the load path) to finish.  `00 00 00 00`-headed records are transforms/materials, not geometry.
| The Lord of the Rings `Cact` actors (RotK, The Third Age x2) | 3 | **the `.scg` levels are SHOC (2026-09-04)** - STOC index + the Tiger Woods stream; terrain and textures read through `ea_obg` (named elements) and `ea_txg` (80-byte entries): 117k triangles / 358 textures on one RotK level; wave 64.  **The `Cact` characters are CRACKED (2026-09-04)** - a `Cact` is a 172-byte actor descriptor (`tACT` + `aRSL` resource list) and the mesh is the `scbm` member: `OBG 01 05` holding a `GCsk` skinned model - s16/1024 model-space bind-pose vertices with per-vertex two-bone weights, s8/64 normals, s16/1024 uvs, GX strip display lists of (pos, nrm, uv) index triples whose materials are literally texture names in the sibling `txf*` groups (`gcrip/formats/ea_cact.py` + plugin; Frodo renders as a T-posed hobbit with Sting, Gimli as a dwarf with axes; triangle counts match declared 16/16, normals unit 4,927/4,928, scale pinned by the ELHE bounding sphere).  **The Third Age's `Ldat` data tag also fell** (u32 size + bare zlib, stream carried across chunks): 341 of 341 resources on e98c02.scg now assemble, where before the disc read as empty.  **`OBG 01 07` fell (2026-09-04 late)** - HEAD slack resync + variable-width corner records (format 3/0/4, read by byte-exact tiling, 38/38 members on e98c02.scg walk to their last byte; element ids are hashes with nothing to bind them to, so geometry ships untextured with vertex colours).  **The `rcb` render skeleton too** - joint count at 0x64 (== max bone byte + 1: 49 Frodo / 60 Gimli), (parent, flag) pairs, inverse-bind 4x4s; all 109 joints land inside their meshes, 62/62 single-bone centroids within 0.35, and `ea_cact` now exports RIGGED scenes behind a no-regression gate (Frodo's main rcb was the off-by-3: a word-padded stored tail, now trimmed).  Open: `bnk` animations, `Crgn`, the element-id -> `txfx` binding, Two Towers' `.msb`/`.msh`/`.BFF` | [formats/ea-shoc-hog.md](formats/ea-shoc-hog.md) |
| Tony Hawk's Pro Skater 3 / 4 scenes | 2 | **the PRE archives open (2026-09-04)**: THPS4's `.prg` is PRE version 0xabcd0002 with 12-byte entries, THPS3's `.pre` the same in little-endian; THPS4's `.tex.ngc` decode through the THUG reader (kon: 396 pictures), THPS3's members are `GCTX` pictures (C8 + RGB5A3 palette after the pixels).  Wave 65.  Open: THPS4's `.scn.ngc` (a 669-material list first, not THUG's header; `gxscan` finds 7 lists in 2.8 MB) and THPS3's scenes | `gcrip/formats/neversoft.py` |
...
| `bin`/`dat` tail | many | **surveyed** - and it is inner-format-bound, not container-bound.  431 of 629 dumped discs produce no triangle-bearing model, and of 158 large bin/dat files on them (audio and video excluded by the manifest's own `kind`) 114 are claimed by no specific container - but the magics **do not cluster**, so there is no single format here to attack.  Closed as audio: `SCHl` (EA, 516 MB over Knockout Kings 2003 and Quidditch World Cup) and `FJF\0` (Sonic Mega Collection's ADX).  **The Kashmir 3-disc cluster is READ (2026-09-04)** - magic `a4 0d 6d 71` on City Racer, Speed Challenge: Jacques Villeneuve and Taxi 3: a little-endian chunk stream of f32 meshes, euler-placed nodes and GameCube pictures - the "entropy 7.2, gxscan finds nothing" verdict was raw float geometry that is not GX.  67k triangles / 138 textures on one track (`gcrip/plugins/kashmir.py`, [formats/kashmir-dat.md](formats/kashmir-dat.md); wave 59).  **Tomb Raider: Legend's `bigfile.dat` is now open AND its models rip (2026-09-04)** - the container was a sorted hash table (617 of 617 sampled members read exactly), the `7c/7d`/`6d` families turned out to be **sounds** (sample rate ~32k/28k, sample count, channels, duration in 30fps frames - `samples/rate == frames/30` exactly), `Mus!` is music, and the `00 00 00 0e` members are **Crystal Dynamics DRM units, fully cracked**: byte-exact tiling on 16/16 (header + records + relocation pairs + unit header + per-section `relocs*8+size` == length), typed sections (0 data / 2 anim / 5 texture / 6 wave / 7 material), and model headers (`04 c2 04 52`) pointing into naked GX display lists - 9-byte vertices `{u8 mtx, u16 pos, u16 nrm, u16 col, u16 uv}` over s16 position / s8 normal / RGBA / u8 uv arrays, which is why `gxscan` was blind (no CP/VAT setup in the lists).  `gcrip/plugins/tr_legend.py` ships: 43 models / 9,011 triangles from 16 sampled units, renders show a bird, a spyglass, a leopard head with whiskers.  Still open there: type-5 texture decoding, skinned assembly (matrix index + pivot list), UV quantisation, `!WAR` | see [formats/bin-dat-tail.md](formats/bin-dat-tail.md) |

## Leads opened 2026-09-04 (mapped, not yet read)

| target | discs | where it stands |
|---|---|---|
| Acclaim `AAAp` geometry | 2 (Vexx, Turok Evolution) | the supertree's nested tables are walked and the 74 `AAAp` members (116 MB) located; header `"AAAp", u32 0x01f00002, u32 off a, u32 off b, u32 32000, u32`, 0x50-byte segment records (two offsets, 8x s16 bbox, count) then an entropy-8 packed vertex stream (columns all near-uniform, so bit-packed/compressed - a DOL decoder is needed.  **Probed 2026-09-04**: three regions - main (0xc8..off_a, 79,656 B), A (off_a..off_b, 11,408 B = 713 x 16), B (off_b..end, 200 B) - and **no standard codec touches any** (zlib x3 window modes, lzo, Okumura LZSS all fail); the `AAAp` tag has no code reference, so a generic loader decompresses it.  Region A's first-of-16 bytes span 1-120 (52 distinct) so it has structure; main and B read as noise (a structure-of-arrays plane split gives random byte-diffs on every plane; and region A (713x16) is itself mostly high-entropy with only two ordered u16 fields per record (256..30963, likely block offsets) - so A is decompression bookkeeping, not vertices.  AAAp is a custom compressor; the only way in is finding the decompressor in main.dol by its ring-buffer/bit-reader signature - the version 0x01f00002 and the `AAAp` tag appear as no code immediate, so a constant search does not locate it.)  The way in is tracing the generic decompressor in `main.dol` (near `NewGeometry`, `USE_VERT_ALPHA` at 0x80267188), not a codec sweep) |
| Taz: Wanted Blitz actors | 1 | **CLOSED 2026-09-04 (evening)** - `gcrip/formats/taz_actor.py` reads the 2002 BlitzTech generation, mapped from cached resources with the header's own oracles (f32 radius @0x9c, min/max bbox pairs @0xa0, resource size @0xbc: 265/267 fit).  Rigid mesh = 6-word record `pos nrm tex clr dl_off dl_size` (dl_size is the padded allocation - clamp); skinned actors have one `(dl_off,dl_size)` table entangled with the skin-influence table, so DLs are found by the CP signature `08 50 00 00 7e 00` every list opens with (496/496 on taztarzan).  Positions AND normals f32 (not s8), stride-8 u16 indices, inline CP/XF loads (XF = skin matrices).  Texture CRCs in 16-byte batch records: rigid `{count,crc,0,0}`, skinned `{crc,0,0,count}`, consumed in prim order.  **267/267 actors extract, 8,847 triangles from the hub pack, 215 scenes with bound textures; geometry verified by wireframe render** (taztarzan is visibly a character).  `plugins/blitz_actor.py` falls back to it when the Bratz layout raises |
| One Piece `FSM_v1.2` `.nex` / `.apf` | 3 (Grand Battle, Grand Adventure, Grand Battle 3) | fixed 0x8000-byte blocks, each `.TPL`-named (`LUF_T__Q00.TPL`), a 0x54-byte header (`u32[10]`) then a `0x0020AF30` body whose count/table words are not the standard TPL fields - a per-game TPL wrapper, still to map |
| Baten Kaitos big `E000` members | 4 (both discs x2) | 4.8-5.7 MB, entropy 8.0, not the LZSS the small text members use - an unknown codec is in front of the character models and pre-rendered backgrounds |
