...
| Taz: Wanted Blitz actors | 1 | the `.gcp` packs expand to 697 members (267 `.tba`, 430 `.tbt`) but every actor reads 0 nodes / 0 meshes: vertex_type 0 (not in `blitz_actor.LAYOUTS`) and the node-tree root pointer at +160 does not resolve for this variant.  Other Blitz `.gcp` discs (Bratz, SpongeBob CFTKK) read, so it is a Taz-specific actor layout |
