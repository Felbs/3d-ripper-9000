...
| Free Radical `gcr` - the **Future Perfect / Second Sight** layout | 2 | **TS2 is closed 2026-09-03** - characters, props, guns and now the levels (`bg/level*.gcr`: the same 0xa0 node records, one a sector, found by the self-pointer at +0x9c; 6-byte vertex-lit vertices; level 11 is 51 sectors / 10,461 triangles / 45 textures).  Future Perfect's props (`trolla_chair_prop.gcr`: `12, ptr header, ptr, ptr`) embed their textures behind the slot table (`ptr, u32, 0, 0x10000000`) and end in s16 data, its characters (`trolla_jp_chr.gcr`) open with three pointers into a 52-byte block; both need the FP DOL | pull the FP and Second Sight DOLs (`pull_top.py`) and find the `GXSetArray` caller that sets an s16 position format; the TS2 DOL's draw routine at `0x8024a454` is the reference |
