...
|---|---|---|---|
| EA `OBG ` terrain | 4 (Tiger Woods PGA Tour 2003, 2004, 2005, 06) | **`TXG ` textures are done** - 2,073 decode, 100% of those found (see [formats/ea-txg.md](formats/ea-txg.md)).  The `SHOC` walk was also ending mid-archive on a bare `FILL` pad; fixing it took 2003 from 60 members to 953, 2004 to 984 and 2005 to 1,364.  Those three discs carry **no `TXG` groups at all** in the first twenty archives - their kinds are `Cact`, `RPNS`, `RLst`, `CAMC`, `tLOD` - so their textures are somewhere else and that is the next question.  `ter` opens `OBG ` with an `ARRA` chunk and `gxscan` finds only 2-3 meshes per member, so the terrain primitives are inside `OBG`/`ARRA` | see [formats/ea-shoc-hog.md](formats/ea-shoc-hog.md) |
...
| Warthog `.hog` codec - tokens `0x80`-`0xDF` only | 3 (Animaniacs, Looney Tunes: Back in Action, Harry Potter and the Sorcerer's Stone) | **two of the three token ranges are solved and verified against real text.**  `0xE0|n` is a literal run of `(n+1)*4`; a token below `0x80` is a match carrying literals - `lit = b & 3` emitted *before* the match, `len = (b>>2) + 3`, `off = next byte + 1`.  `frontend_cog1.lvl` decodes to `level CRLF { CRLF TAB name({cactus}) CRLF TAB acount(4) CRLF TAB pcount(0) CRLF TAB` and stops at the first token >= `0x80`, at output byte 52.  What is left is that one range, and the test vector is exact: the next bytes are `87 40 0b XX` repeating with only `XX` changing (`s`, `t`, `b`, `o`) and each unit must emit that letter then `count(0) CRLF TAB`.  The offset cannot be the byte after the token (`0x40+1` = 65 with only 52 bytes of output), so `0b` (11) is almost certainly the offset with no bias.  The high form takes **two operands**, and searching that shape yields real text - `lit=(t>>4)&3, len=a+3, off=b` decodes `triggers.txt` to `triggers CRLF { CRLF TAB trigger ... name(ProximityIn) ... setting(1, number, range` - but every candidate **repeats a fragment**, which is what a match does when its length overruns the cycle it copies: `a+3` is 67 and copying 67 from eleven back replays an eleven-byte cycle six times.  So the length is not the whole of `a`; the unit must emit 11 bytes.  The solved half decodes 44 of 4,663 members outright, so the value so far is the grammar, not the yield.  Behind it: 29,021 `.bmsh` meshes and 36,156 `.btga` textures | see [formats/warthog-hog.md](formats/warthog-hog.md) |
