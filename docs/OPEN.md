...
| format | discs | state |
|---|---|---|
| Darkened Skye `.SKX` models | 1 | textures ship (4,340 `GCT`) and the `.pak` container ships (255 `.SKX` extracted).  `SKX` is big-endian: size at +8 matches exactly, the two header offsets are name pointers (`AwGargSt#120`, `Gargoyle`), a directory at 0x25de mixes counts and offsets, and there are index lists of BE `u16` triples reaching 722.  **Every float array is normalised to max 1.0**, so the geometry is unit-space and the header matrix and placement position it - there is no world-scale array to find.  Next: reconcile the 722 index range against the array lengths | 
