...
**Systems:** 10 working, 6 partial, 0 not started.  
**Story:** 375 of 415 mined steps reachable (14 partial, 26 missing) across 18 chapters: outset, fortress, dragonroost, forbiddenwoods, jabun, towerofgods, hyrule, temples, fortress2, ganon, ganontower, caves, gallery, houses, labyrinths, minigames, triforce, windfall.
...
flowchart LR
    ...
    class G0_2 ok;
    G0 --> G0_3["Sailing<br/><i>boat physics, sail, wind, wave_max from the real MULT values, RT</i>"]
    class G0_3 partial;
    GAME --> G1["Link"]
    ...
    class G3_1 partial;
    G3 --> G3_2["Shading looks<br/><i>five looks on one F6 cycle (--shade=toon|hybrid|clay|paper|pbr):</i>"]
    class G3_2 partial;
    G3 --> G3_3["Particles<br/><i>common.jpc parsed (193 effects, 96 textures through the existing</i>"]
    class G3_3 partial;
    GAME --> G4["Progress"]
...
| Particles | partial | common.jpc parsed (193 effects, 96 textures through the existing BTI decoder) and driven by Game.fx with the mix(env, prm, mask) TEV preset; enemy deaths use the real SIBOUBAKUEN + SIBOUFLASH set. One bank of 58, no child emitters, no FLD1 fields yet |
| Shading looks | partial | five looks on one F6 cycle (--shade=toon|hybrid|clay|paper|pbr): the game's own toon.bti ramp, untouched; hybrid = that ramp as diffuse + GGX specular, Schlick Fresnel, a Charlie/Ashikhmin sheen lobe (KHR_materials_sheen - Godot has none, so it is written) and clearcoat, by material class from ww_materials.json with metal curated only; clay and paper as wrapped-diffuse looks with procedural grain. NOT yet seen by human eyes, and BTK material animation is still unparsed |
...
| Music | partial | 167 stage songs with vibrato, per-track fx send and the pitch oscillator mined from JAudio; footsteps play the real Footnote bank waves for the 28 ground materials (the collision is split by sound_id so the ray hit names the surface). Every other effect (sword, items, voices) is still silent: seStart is a stub, so their id -> bank/program mapping is not recoverable from source |
| Sailing | partial | boat physics, sail, wind, wave_max from the real MULT values, RTBL room streaming (1 of 49 room subtrees drawn), and - new - an ocean surface that actually renders: a 65x65 heightfield of 800-unit cells around Link with the four cosines evaluated in the vertex shader from the same SEA_WAVES the buoyancy reads. Cel-shaded; a PBR water mode is a second variant of this mesh. No cannon, crane or cyclones |
...
flowchart TD
    ...
    subgraph labyrinths["labyrinths"]
        ...
        icering_melt["icering_melt<br/>MELT_ICE<br/><i>hit - OK</i>"]
        ...
        firemountain_freeze["firemountain_freeze<br/>FREEZE_VOLCANO<br/><i>hit - OK</i>"]
        ...
        cabana_hammer_doors["cabana_hammer_doors<br/>DEFAULT_STOP_OPEN<br/><i>hit - OK</i>"]
        ...
        needlerock_six_torches["needlerock_six_torches<br/>DEFAULT_TREASURE_APPEAR<br/><i>hit - OK</i>"]
        ...
        boating_course_hit_switches["boating_course_hit_switches<br/>DEFAULT_TREASURE_APPEAR<br/><i>hit - OK</i>"]
        ...
        tf05_hammer_toggle_puzzle["tf05_hammer_toggle_puzzle<br/><i>hit - OK</i>"]
...
| jabun | jab_blow_open_the_cave | hit | ajav_uzu |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
| labyrinths | icering_melt | hit | MELT_ICE |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
| labyrinths | firemountain_freeze | hit | FREEZE_VOLCANO |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
| labyrinths | cabana_hammer_doors | hit | DEFAULT_STOP_OPEN |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
| labyrinths | needlerock_six_torches | hit | DEFAULT_TREASURE_APPEAR |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
| labyrinths | boating_course_hit_switches | hit | DEFAULT_TREASURE_APPEAR |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
| labyrinths | tf05_hammer_toggle_puzzle | hit |  |  | ok | actors/hit_object.gd - take_hit stages that each order an event, positioned from the stage's own placement records (the Ajav wall over Jabun's cave breaks in three stages); plus actors/hit_switch.gd for the objects that accept exactly ONE attack kind and raise a switch - 780 bonbori, 50 SW_HIT0, 26 MhmrSW0 and the single Qdghd and Ykzyg, with take_hit carrying the attack kind |
...
- **Breakable objects without geometry** - 2 steps: the mechanism works, but this step has no structured hit block, so nothing is spawned for it
