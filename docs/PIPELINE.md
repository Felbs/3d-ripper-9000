...
flowchart LR
    ...
    RIP -->|"format a plugin claims<br/>(gcrip.plugins: retro, hsd, gma,<br/>jade, re4, ea, eagl, ebo, p3d, mdl2, mdl3, krome_gc01, eurocom, hsf, sa2b, sadx, ninja_gc, billy, nu2, ttdisp, hgo, unreal, openspace, afs, lpac, melee, dbl, blitz, blitz_tex, bolt, bolt_model, bolt_mat, mk_ssf, treyarch_pak, ngl_mesh, nlg_glg, ea_la, edge_arc, edge_dataset, edge_model, edge_tex, dds_pack, fsta, agg, asb_tex, bmp, hff, png, frd_gct, frd_gcr, frd_pak, auf_ngc, ft_pak, gct, jam2, kceo, ljam, skye_pak, skx, toc_tim, toc_wad, vc_dat, vc_pack, vc_scene, tpl, mdgc, xmdl, u8, res (rdms meshes, bmsh characters, surf textures), kashmir, fang_mst, neko_lz, acclaim_tre, ubi_fat, ubi_geoobj, totemtech, acclaim_gdf, acclaim_skn, rw_native, cod_rws, a2m_gcr, a2m_hg, nin_gpl, pov_model, pov_level, yukes_pac, yukes_tex, yukes_yobj, zip, pod, tr_pkg, tr_tex, tr_smf, tr_dfm, tr_smb, climax_bad, climax_bog, climax_rom, climax_row, hsd, renderware, …)"| PLUG["3b  Plugin parse<br/>plugin.extract → ripcore Scene"]
...
flowchart LR
    subgraph EA["Electronic Arts"]
        ...
        SHOC["SHOC .hog / .scg<br/>plugins.shoc"] --> RCMP["Rdat = rcmp LZ, Ldat = zlib<br/>formats.ea_rcmp, shoc"] --> OBG["OBG terrain / TXG textures<br/>plugins.ea_obg, ea_txg"]
        SHOC --> CACT["Cact actors -> GCsk characters<br/>plugins.ea_cact"]
        NGC[".ngc index<br/>formats.totemtech"] --> DGC["TotemTech .dgc record chain<br/>plugins.totemtech"]
        BIG --> EBO["EBO .ebo<br/>plugins.ebo"]
    end
    subgraph POV["Point of View"]
        WAD["inline .wad records<br/>plugins.toc_wad"] --> PHM["PHM s_model<br/>plugins.pov_model"] & TIMR["TIM s_texture_header<br/>formats.pov_model"] & TG["TG_&lt;phase&gt; layouts<br/>plugins.pov_level"]
        TIMR -.->|"by texture-def name"| PHM
        PHM -.->|"by wad record id + world-space rest"| TG
    end
    subgraph NINSDK["Nintendo SDK character pipeline"]
        GPL[".gpl geometry palettes<br/>plugins.nin_gpl"]
        TPLN[".tpl palettes (named / split)"] -.->|"by name + image index"| GPL
    end
    ...
    subgraph BOLT["Mass Media"]
        BLT["BOLT .BLT archives + LZ<br/>plugins.bolt"] --> BMDL["node tree + MESH::Load .bmdl<br/>plugins.bolt_model"]
        BLT --> BMAT["material lists .bmat<br/>plugins.bolt_mat"]
        BMAT -.->|"textures by material index"| BMDL
    end
    subgraph TREYARCH["Treyarch NGL"]
    ...
    subgraph NLG["Next Level Games"]
        ...
        GLT[".glt PTLG bundles<br/>formats.nlg_gl"] -.->|"by texture hash"| GLG
    end
    subgraph MIDWAY["Midway"]
        SSF["SEC .ssf archives<br/>plugins.mk_ssf"] --> MKDFF["in-place clumps .mkdff<br/>plugins.renderware"]
        SSF --> MKTEX["textures .mktex<br/>formats.mk_ssf"] -.->|"by raster name"| MKDFF
    end
    subgraph EOR["Edge of Reality"]
        IND["index.ind + .arc<br/>plugins.edge_arc"] --> EMDL["MODL models<br/>plugins.edge_model"]
        IND --> ETEX["TXFL textures<br/>plugins.edge_tex"]
        IND --> EDS["Datasets packs<br/>plugins.edge_dataset"] --> EMDL & ETEX
        IND -.->|"SHDR by name hash"| EMDL
    end
    ...
    subgraph KROME["Krome (Merkury)"]
        ...
        RKV --> GC01["GC01 .mdl + .mdg (Jet Fusion)<br/>24-byte display-list vertices<br/>plugins.krome_gc01"]
    end
    ...
    subgraph KONAMI["Konami (TMNT 1-3)"]
        ...
        AFS --> XMDL["XMDL / NTGC models<br/>GRPV directory, 32-byte f32 vertex<br/>plugins.xmdl"] & DDSP["DDS packs -> GX CMPR<br/>plugins.dds_pack"]
    ...
    subgraph OTHER["Capcom / Ubisoft / Neversoft / Blitz / Terminal Reality"]
        ...
        CBOG -.->|"by material name"| CROM & CROW
        HOG["Warthog WART3.00 .hog<br/>records at +24, dir+file names<br/>plugins.wart_hog"] --> HOGM["private LZ, 4 token forms<br/>10/14/16-bit windows<br/>98-99% of members"] --> BTGA["btga textures<br/>96-byte header + GX mip chain<br/>plugins.wart_btga"] --> BMSH["bmsh meshes<br/>section-table chain + GX display lists<br/>plugins.wart_bmsh"] --> HOGX["16/16 sampled members<br/>6,672 triangles, positions + UVs"]
...
flowchart TD
    ...
    D -->|"retro / hsd / gma / pikmin / lm / sfa / jade /<br/>re4 / neversoft / renderware / ea / eagl / ebo / p3d / mdl2 / mdl3 / eurocom / hsf / sa2b / ninja_gc / billy / ttdisp / unreal / openspace / ttyd / feporr / mula / totemtech / acclaim_gdf / acclaim_skn / rw_native"| R["real parser<br/>meshes + materials + textures<br/>(+ rig, + clips where the format has them)"]
