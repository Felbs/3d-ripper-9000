...
Layout (big-endian throughout)::
    ...
    nnodes x node record (224 bytes):
        ...
        f32 pivot[3]  f32 maxabs[3]  f32 radius  s32 parent (-1 root)
    nnodes x mesh section, in node order:
        ...
        nsub x submesh:
            ...
            u32 ndl(1)  u32 0  u32 dl_size  GX display list
...
u32 size`` + tiled GX data (C8 when paletted, CMPR otherwise).  Materials ``.gcm`` are
...
class Node:
    name: str
    matrix: np.ndarray  # (4,4) f32 row-major
...
class Model:
    version: int
    nodes: list[Node]
    end: int  # byte offset the parse stopped at (== len(data) for a clean file)
...
def _display_list(dl: bytes, nfields: int) -> list[tuple[int, np.ndarray]]:
    prims = []
    p = 0
    n = len(dl)
    while p < n:
        ...
        need = count * nfields * 2
        if p + 3 + need > n:
            raise GcgError("display list overruns its size")
        idx = np.frombuffer(dl, ">u2", count * nfields, p + 3).reshape(count, nfields)
...
def _submesh(data: bytes, q: int) -> tuple[Submesh, int]:
    ...
    ndl, _zero, dl_size = struct.unpack_from(">III", data, q)
    q += 12
    if ndl != 1 or q + dl_size > n:
        raise GcgError(f"display list count {ndl} / size {dl_size} not plausible")
    prims = _display_list(data[q : q + dl_size], fields)
    q += dl_size
    for _, idx in prims:
        if len(idx) and int(idx.max()) >= nverts:
            raise GcgError("display-list index past the vertex count")
    return Submesh(material, positions, normals, colors, uvs, prims, float(radius)), q
...
def parse(data: bytes) -> Model:
    ...
    for node in nodes:
        ...
        node.materials = [_cstr(data[p + i * 64 : p + (i + 1) * 64]) for i in range(nmat)]
        p += nmat * 64
        ...
        for _ in range(nsub):
            ...
            node.submeshes.append(sub)
    return Model(version, nodes, p)
...
def decode_gct(data: bytes) -> Texture:
    ...
    p += 12
    if lw != w or lh != h:
        raise GcgError("first mip level does not match the header size")
    body = data[p : p + size]
