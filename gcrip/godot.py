...
func _ready() -> void:
    ...
    if load_game():
        ...
        if not selftest and shot_actor == "" and not door_test and not story_test and menu_test == "" and event_test == "" and not newgame_test:
...
func _process(delta: float) -> void:
    if shot_actor != "":
        _shot_tick()
    if newgame_test or event_test != "":
        # tests have no player: tap A so text boxes advance instead of holding the timeline
        auto_a += 1
        if dialog_open and auto_a % 12 == 0:
    ...
    autosave_frames += 1
    if autosave_frames >= 30 * 60 and not dialog_open and not event_running and not selftest and shot_actor == "" and not door_test and not story_test and menu_test == "" and event_test == "":
        autosave_frames = 0
        save_game("autosave")
...
func story_event_done(name: String) -> void:
    ...
    for step in story.get("steps", []):
        ...
        if ev != name and stb != name and stb != name + ".stb":
            continue
...
def export_godot(
    ...
    *,
    ...
    if story_src.exists():
        shutil.copyfile(story_src, out_dir / "story.json")
